...
// ReSharper restore FieldCanBeMadeReadOnly.Global
        ...
        public virtual OLVListItem GetItem(int index) {
            ...
            //ORIGINAL_FROM_VENDOR return (OLVListItem)this.Items[index];
            
            //SQ1_IMPLEMENTATION: catching GUI-initiated model.READ while model.WRITE hasn't been finished yet 
            OLVListItem ret = null;
			if (null == this.Items) return ret;

            try {
				object item = this.Items[index];
				if (null != item) {
					ret = item as OLVListItem;
				} else {
					string msg = "WEIRDO";
				}
            } catch (Exception ex) {
            	#if DEBUG
            	string msig = "ObjectListView.Name[" + this.Name + "].Count=[" + this.GetItemCount() + "].GetItem(" + index + ")";
            	string msg = "WHY_INTRODUCED: observed this.Items.Count5=6, index=5 but this.Roots.IList had only 0...4 elements (ExceptionControl.olvTreeExceptions)";
            	msg += "1) ExceptionControl should fill treeExceptions only when Visible() <= flushListToTree() does it now";
            	msg += "2) Serializer.Initialize() was with fileCreate=false => default ctor() not sync'ed with owning Control => added bool fileCreated for further sync";
            	Debugger.Break();
            	#endif
            }
            return ret;
