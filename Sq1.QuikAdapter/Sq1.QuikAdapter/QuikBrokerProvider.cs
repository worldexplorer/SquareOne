...
namespace Sq1.QuikAdapter {
	[DataContract]
	public class QuikBrokerProvider : BrokerProvider {
		QuikBrokerEditor editor;
		...
		public QuikBrokerProvider() : base() {		// base() will be invoked anyways by .NET, just wanna make it obvious (reminder)
		...
		public override BrokerEditor BrokerEditorInitialize(IDataSourceEditor dataSourceEditor) {
			...
			base.brokerEditorInstance = new QuikBrokerEditor(this, dataSourceEditor);
		...
		QuikTerminalConnectionState previousConnectionState = QuikTerminalConnectionState.None;
		int identicalConnectionStatesReported = 0;
		int identicalConnectionStatesReportedLimit = 3;
		public void callbackTerminalConnectionStateUpdated(QuikTerminalConnectionState state, string message) {
			if (this.previousConnectionState == QuikTerminalConnectionState.None) {
			...
			if (state != QuikTerminalConnectionState.DllConnected) {
		...
		public override void OrderPreSubmitEnrichBrokerSpecificInjection(Order order) {
			...
			QuikQuote quikQuote = QuikQuote.SafeUpcast(order.Alert.QuoteCreatedThisAlert);
			if (order.Alert.PositionLongShortFromDirection == PositionLongShort.Long) {
		...
		public override string ModifyOrderTypeAccordingToMarketOrderAsBrokerSpecificInjection(Order order) {
			...
			if (order.Alert.QuoteCreatedThisAlert is QuikQuote == false) {
				msg = "Should be of a type Sq1.QuikAdapter.QuikQuote instead of Sq1.Core.DataTypes.Quote: "
					+ order.Alert.QuoteCreatedThisAlert;
				throw new Exception(msg);
			}
			QuikQuote quikQuote = order.Alert.QuoteCreatedThisAlert as QuikQuote;
			if (order.Alert.PositionLongShortFromDirection == PositionLongShort.Long) {
