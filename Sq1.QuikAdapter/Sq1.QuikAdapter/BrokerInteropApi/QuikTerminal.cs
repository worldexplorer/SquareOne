...
namespace Sq1.QuikAdapter.BrokerInteropApi {
	public class QuikTerminal {
		protected QuikBrokerProvider quikBrokerProvider;
			...
			lock (tryingToConnectDllLock) {
				...
				try {
					...
					if (ret != Trans2Quik.Result.SUCCESS) {
						quikBrokerProvider.callbackTerminalConnectionStateUpdated(QuikTerminalConnectionState.None, this.callbackErrorMsg.ToString());
					...
					ret = Trans2Quik.CONNECT(quikBrokerProvider.QuikFolder, out error, this.callbackErrorMsg, this.callbackErrorMsg.Capacity);
					if (ret != Trans2Quik.Result.SUCCESS && ret != Trans2Quik.Result.ALREADY_CONNECTED_TO_QUIK) {
						quikBrokerProvider.callbackTerminalConnectionStateUpdated(QuikTerminalConnectionState.None, this.callbackErrorMsg.ToString());
						msg = "3/5 FAILED: CONNECT(" + quikBrokerProvider.QuikFolder + "): error[" + error + "][" + callbackErrorMsg + "] ret[" + ret + "] != SUCCESS";
						Assembler.PopupException(msg);
						//throw new Exception(msg);
						return;
					}
					...
					if (ret != Trans2Quik.Result.SUCCESS) {
						quikBrokerProvider.callbackTerminalConnectionStateUpdated(QuikTerminalConnectionState.DllConnected, "can not UNSUBSCRIBE_ORDERS()");
					...
					if (ret != Trans2Quik.Result.SUCCESS) {
						quikBrokerProvider.callbackTerminalConnectionStateUpdated(QuikTerminalConnectionState.DllConnected, "can not UNSUBSCRIBE_TRADES()");
					...
					*/
					connectionTimer.Change(Timeout.Infinite, Timeout.Infinite);
					msg = "#" + connectionAttempts + "//timeoutInfinite QuikTerminal(" + this.DllName + ") 2/2 " + QuikTerminalConnectionState.ConnectedUnsubscribed;
					quikBrokerProvider.callbackTerminalConnectionStateUpdated(QuikTerminalConnectionState.ConnectedUnsubscribed, msg);
					Assembler.PopupException(msg);
				} catch (Exception e) {
					quikBrokerProvider.callbackTerminalConnectionStateUpdated(QuikTerminalConnectionState.Exception, e.Message);
		...
		public void Subscribe(string SecCode, string ClassCode) {
			if (DllConnected == false) {
				string msg = "can not RegisterQuoteConsumer(" + SecCode + "," + ClassCode + "): DLL[" + DllName + "] must be connected first [" + quikBrokerProvider.QuikFolder + "]";
				Assembler.PopupException(msg);
				//quikBrokerProvider.ExceptionDialog.PopupException(new Exception(msg));
				throw new Exception(msg);
			}
			...
			quikBrokerProvider.callbackTerminalConnectionStateUpdated(QuikTerminalConnectionState.ConnectedSubscribed,
		...
		public void Unsubscribe(string SecCode, string ClassCode) {
			...
			QuikTerminalConnectionState state = (SymbolClassSubscribers[key] > 0)
				? QuikTerminalConnectionState.ConnectedSubscribed : QuikTerminalConnectionState.ConnectedUnsubscribed;
			quikBrokerProvider.callbackTerminalConnectionStateUpdated(state,
		...
		void callbackConnectionStatus(Trans2Quik.Result evnt, int err, string m) {
			switch (evnt) {
				case Trans2Quik.Result.QUIK_DISCONNECTED:
					connectionTimer.Change(0, quikBrokerProvider.ReconnectTimeoutMillis);
					Assembler.PopupException("CallbackConnectionStatus[" + evnt + "]  err[" + err + "] m[" + m + "]: connectionTimer.Change(0, " + quikBrokerProvider.ReconnectTimeoutMillis + ")");
					break;
...
*/
		protected void CallbackTradeStatus(int nMode, double trade_id, double SernoExchange,
			...
			//Order orderExecuted = this.quikBrokerProvider.OrderProcessor.DataSnapshot.OrdersPending.FindBySernoExchange((long)SernoExchange);
			Order orderExecuted = this.quikBrokerProvider.OrderProcessor.DataSnapshot.OrdersAll.FindBySernoExchange((long)SernoExchange);
			...
			//quikBrokerProvider.PutOwnTrade(new OwnTrade(
			...
			quikBrokerProvider.CallbackTradeStateReceivedQuik((long)SernoExchange, tradeDate, 
...
*/
		protected virtual void CallbackOrderStatus(int nMode, int GUID, double SernoExchange,
			...
			string msgDebug = " price[" + priceFilled + "] status[" + status + "]"
				...
				;
			OrderList orders = quikBrokerProvider.OrderProcessor.DataSnapshot.OrdersPending;
			Order orderExecuted = orders.FindByGUID(GUID.ToString());
			if (orderExecuted == null) {
				orders = quikBrokerProvider.OrderProcessor.DataSnapshot.OrdersSubmitting;
				orderExecuted = orders.FindByGUID(GUID.ToString());
			}
			if (orderExecuted == null) {
				orders = quikBrokerProvider.OrderProcessor.DataSnapshot.OrdersAll;
				orderExecuted = orders.FindByGUID(GUID.ToString());
			}
			if (orderExecuted == null) {
				msgError += " Order not found Guid[" + GUID + "] ; orderSernos=["
					+ quikBrokerProvider.OrderProcessor.DataSnapshot.OrdersPending.SessionSernos
					+ "] Count=[" + orders.Count + "]";
			} else {
			...
			quikBrokerProvider.CallbackOrderStateReceivedQuik(WldStatus, GUID.ToString(), (long)SernoExchange,
...
*/
		protected void CallbackTransactionReply(Trans2Quik.Result r, int err, int rc, int GUID, double SernoExchange, string msgQuik) {
			string msg = "QuikTerminal(" + this.DllName + ")::CallbackTransactionReply() "
				...
				;
			var orders = quikBrokerProvider.OrderProcessor.DataSnapshot.OrdersPending;
			...
			if (r == Trans2Quik.Result.SUCCESS && rc == 3) {
				...
				Assembler.PopupException(msg2 + msg);
				//quikTransactionsAttemptedLog.Put(msg);
				//quikBrokerProvider.callbackTerminalReceivedOrderState(OrderStatus.ErrorSubmittingBroker, orderSubmitting.GUID,
			...
			quikBrokerProvider.OrderProcessor.UpdateOrderStateAndPostProcess(orderSubmitting, newOrderState);
		}
		public virtual void SendTransactionOrderAsync(char opBuySell, char typeMarketLimitStop,
			...
			if (!IsSubscribed(SecCode, ClassCode)) {
				try {
					...
					//this.quikBrokerProvider.StatusReporter.PopupException(new Exception(msgSumbittedOut, e));
			...
			this.quikBrokerProvider.OrderProcessor.UpdateOrderStateByGuidNoPostProcess(GUID, orderStateOut, trans);
		...
		protected string getOrderCommand(char opBuySell, char typeMarketLimitStop,
			...
			string actionTypePriceStop = "ACTION=NEW_ORDER;TYPE=" + typeMarketLimitStop + ";PRICE=" + Price.GetRaw(price) + ";";
			if (typeMarketLimitStop == 'S') {
				double stopPrice = price;
				actionTypePriceStop = "ACTION=NEW_STOP_ORDER;PRICE=" + Price.GetRaw(price) + ";STOPPRICE=" + Price.GetRaw(stopPrice) + ";";
			}
		...
		protected virtual string getAccountClientCode(string ClassCode) {
			string ret = "";
			if (quikBrokerProvider == null) {
				Assembler.PopupException("can't set ACCOUNT=XXX and CLIENT_CODE=YYY: quikBrokerProvider=null");
			} else {
				if (quikBrokerProvider.AccountAutoPropagate == null) {
					Assembler.PopupException("can't set ACCOUNT=XXX: quikBrokerProvider[" + quikBrokerProvider + "].Account=null");
				} else {
					string account = quikBrokerProvider.AccountAutoPropagate.AccountNumber;
					if (ClassCode != "SPBFUT" && quikBrokerProvider.AccountMicexAutoPopulated != null) {
						account = quikBrokerProvider.AccountMicexAutoPopulated.AccountNumber;
					}
					ret += "ACCOUNT=" + account + ";";
				}
				ret += "CLIENT_CODE=" + quikBrokerProvider.QuikClientCode + ";";
			}
			return ret;
		}
		public virtual void SendTransactionOrderKillAsync(string SecCode, string ClassCode,
			...
			this.quikBrokerProvider.OrderProcessor.UpdateOrderStateByGuidNoPostProcess(KillerGUID, OrderState.KillerSubmitting, trans);
		...
		public virtual void sendTransactionKillAll(string SecCode, string ClassCode, string GUID, out string msgSumbitted) {
			...
			//	+ "ACCOUNT=" + quikBrokerProvider.SettingsManager.Get(
			//		"QuikStreamingProvider.QuikAccount", "SPBFUTxxxxx") + ";"
			//	//+ "CLIENT_CODE=" + quikBrokerProvider.SettingsManager.Get(
