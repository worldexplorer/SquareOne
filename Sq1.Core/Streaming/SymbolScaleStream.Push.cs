...
namespace Sq1.Core.Streaming {
	public partial class SymbolScaleStream {
		...
		public	UnattachedStreamingBar_factory								UnattachedStreamingBar_factory	{ get; protected set; }
				Dictionary<StreamingConsumer, BinderAttacherPerConsumer>	binderPerConsumer;

		public void PushQuote_toConsumers(Quote quoteDequeued_singleInstance) { lock (this.lockConsumersQuote) {
			...
			if (quotePrev != null && quoteDequeued_singleInstance.ServerTime < quotePrev.ServerTime) {
				Bar streamingBar = this.UnattachedStreamingBar_factory.BarStreaming_unattached;
			...
			foreach (StreamingConsumer consumer in this.ConsumersAll) {
				...
				try {
					quote_clonedBoundAttached = this.quoteCloned_bindAttach(quoteDequeued_singleInstance, consumer);
				} catch (Exception ex) {
				...
				if (this.ConsumersQuote.Contains(consumer)) {
					try {
						consumer.ConsumeQuoteOfStreamingBar(quoteClone_boundAttached);
		...
		void bar_lastStaticFormedAttached_consume(Quote quoteClone_sernoEnriched_withStreamingBarUnattachedToParents, StreamingConsumer barConsumer) {
			string msig = " //bar_lastStaticFormed_attachConsume() " + this.ToString();
			Bar barStreamingUnattached_clonedFromFactory = this.UnattachedStreamingBar_factory.BarStreaming_unattached.Clone();
			...
			BinderAttacherPerConsumer binderForConsumer = this.binderPerConsumer[barConsumer];
			...
			try {
				if (binderForConsumer.Consumer is ChartStreamingConsumer &&
			...
			try {
				...
				if (barStaticLast == null) {
					...
					continue;
						...
						bool barLastStaticFormedIsValid		= !this.UnattachedStreamingBar_factory.BarLastFormed_unattached_notYetFormed;
						bool streamingBarReadyToSpawn		=  this.UnattachedStreamingBar_factory.BarStreaming_unattached.DateTimeOpen
																		< quote_clonedBoundAttached.ServerTime;
						bool streamingBarWronglyRestoredAfterBacktest =
							this.UnattachedStreamingBar_factory.BarStreaming_unattached.DateTimeOpen
								> quote_clonedBoundAttached.ServerTime;
				...
				barConsumer.ConsumeBarLastStatic_justFormed_whileStreamingBarWithOneQuote_alreadyAppended(barStaticLast, quoteBound_toStreamingBarUnattached);
								...
								string msg = "I_REFUSE_BIND_AND_PUSH_LAST_STATIC_FORMED [" + this.UnattachedStreamingBar_factory.BarLastFormedUnattached_nullUnsafe + "]"
									...
									+ " for " + this.UnattachedStreamingBar_factory;
								//Assembler.PopupException(msg, null, false);
							}
						}
			} catch (Exception ex) {
		...
		void quoteCloned_bindAttach(Quote quoteCloned_intrabarSernoEnriched_unbound, StreamingConsumer quoteConsumer) {
			...
			// clone 
			Quote quoteCloned_intrabarSernoEnriched_unbound = this.UnattachedStreamingBar_factory.
			...
			BinderAttacherPerConsumer binderForConsumer = this.binderPerConsumer[quoteConsumer];
