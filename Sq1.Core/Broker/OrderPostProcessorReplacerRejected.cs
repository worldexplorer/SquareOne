...
namespace Sq1.Core.Broker {
	public class OrderPostProcessorReplacerRejected : OrderPostProcessorReplacer {
		...
		protected bool Replace_AnyOrder_withSameSlippage_killWithoutHook(Order anyOrderRejected_willBeKilled_andReplaced) {
			bool replacementScheduled = false;
			string msig = " //Replace_AnyOrder_withSameSlippage_killWithoutHook(" + anyOrderRejected_willBeKilled_andReplaced + ")";
			...
			try {
				...
				if (shallKill == false) {
					...
					bool killerEmitted = this.OrderProcessor.Emit_killOrderPending_usingKiller(anyOrderRejected_willBeKilled_andReplaced, msig);
				}
			} catch (Exception ex) {
			...
			switch (anyOrderRejected_willBeKilled_andReplaced.State) {
				...
				default:
					...
					this.OrderProcessor.AppendMessage_propagateToGui(anyOrderRejected_willBeKilled_andReplaced, msg3);
					return replacementScheduled;
			}
			...
			try {
				...
				if (shallReplace == false) {
					...
					replacementScheduled = this.replaceOrder_withSameSlippage(anyOrderRejected_willBeKilled_andReplaced, null);
				}
			} catch (Exception ex) {
				Assembler.PopupException(msig, ex, false);
			}
