...
namespace Sq1.Core.Sequencing {
	public class ReusableExecutor : ScriptExecutor/*, IDisposable*/  {
		...
		public ReusableExecutor(string reasonToExist, ScriptExecutor scriptExecutor) : base(reasonToExist) {
			...
			base.SetBars(scriptExecutor.Bars.SafeCopy_forEachReusableExecutor(reasonToExist), true);	// frozen, streamingBar detached
		...
		// invoked many times in ReusableExecutorsPool.spawnToReachTotalNr(int threadsToUse) to spawn executorToBeSpawned and add to this.executorsIdlingNow
		internal ReusableExecutor SpawnEthalonForEachThread_forEachReusableExecutor_inSequencerPool(string reasonToExist) {
			//v1 I_DONT_WANT_TO_RESET_AGAIN_base.Livesimulator=null
			ReusableExecutor clone = new ReusableExecutor(reasonToExist, this);
			//v2 HOW_DO_YOU_CREATE_INDIVIDUAL_BACKTESTERS_THIS_WAY??? ReusableExecutor clone = (ReusableExecutor) base.MemberwiseClone();
		...
		internal void InitializeAndPushParameters_toDefaultContextCurrent_fromNextSequenced(ContextScript ctxNext) {
			...
			//MULTITHREADING_ISSUE__YOU_MUST_PASS_CLONE_AND_THEN_LET_OTHER_REUSABLE_EXECUTOR_TO_RUN_ANOTHER_BACKTEST
			...
			base.Strategy.ScriptContextCurrent.AbsorbOnlyScriptAndIndicatorParameterCurrentValues_toReusableFromSequencer(ctxNext);
			...
			#if DEBUG
			string copyFromCtx =							ctxNext.ScriptAndIndicatorParameters_mergedUncloned_forSequencerByName_AsString;
			string copyToCtx   = base.Strategy.ScriptContextCurrent.ScriptAndIndicatorParameters_mergedUncloned_forSequencerByName_AsString;
