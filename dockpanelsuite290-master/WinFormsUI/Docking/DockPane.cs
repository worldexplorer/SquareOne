...
namespace WeifenLuo.WinFormsUI.Docking {
	...
	public partial class DockPane : UserControl, IDockDragSource {
		...
		public virtual IDockContent ActiveContent {
			...
			set {
				// originally, NPE protection for LoadFromXml() but ValidateActiveContent() sets ActiveContent=null
				//if (value == null) return;
				if (ActiveContent == value) return;
				if (value != null) {
					if (DisplayingContents.Contains(value) == false) {
						string msg = "I didn't find the spot where DisplayingContents.Add(anything) happens!";
						//throw (new InvalidOperationException(Strings.DockPane_ActiveContent_InvalidValue));
					//} else {
					//	int index = DisplayingContents.IndexOf(value);
					//	string msg = "index=[" + index + "] while DisplayingContents.Count[" + DisplayingContents.Count + "]";
					}
				} else {
