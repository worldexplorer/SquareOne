...
namespace WeifenLuo.WinFormsUI.Docking {
	...
	public class DockContentHandler : IDisposable, IDockDragSource {
		...
		public DockPanel DockPanel {
			...
			set {
				if (m_dockPanel == value) {
					string msg = "if you have MainForm.PersistStringInstantiator() down the stack,"
						+ " remove IDockContent.Show() for an instantiated form"
						+ " and call it after whole layout will be restored!!!"
						+ " Persistor.LoadFromXml() will throw if dockPanel.Content.Count < contentUnserialized.Count";
					// added but decided not to use AddContent() since DockSample was throwing error on "m_SolitionExplorer.DockContent = null"... wtf
					// and this code runs on MainForm.Dispose() during app close
					//m_dockPanel.AddContent(Content);
					return;
				}
				Pane = null;
				if (m_dockPanel != null) {
					string msg = "removing on DockPane.Close_Click() //why do you have to remove it? Huston will miss it later!";
				...
				if (m_dockPanel != null) {
					...
					string msg = "removing on DockPane.Close_Click() //AddContent(Content) wasn't invoked => Huston will have problems";
				}
			}
		}
		...
		public string TabText {
			...
			set {
				if (m_tabText == value) return;
				m_tabText = value;
				if (Pane != null) Pane.RefreshChanges();
			}
		}
		...
		public DockState VisibleState {
			...
			set { if (m_visibleState == value) return; }
		}
